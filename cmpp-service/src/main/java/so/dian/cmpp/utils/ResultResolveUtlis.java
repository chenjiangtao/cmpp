...
public class ResultResolveUtlis {
	...
	@Autowired
	CmppSubmitMsgThread cmppSubmitMsgThread;
	...
	public static String cmppMessageResult(MessageBean bean) {
		...
		stringBuffer
		.append(result).
		append("sequenceId=").append(bean.getSequenceId()).
		append(", commondId=").append(bean.getCommandId()).
	...
	public String cmppConnectResp(MsgRespThreadBean respThreadBean) throws Exception {
		...
		if (CMPPConstants.status.status_0.equals(String.valueOf(statut))) {
			...
			logger.info("开始启动发送短信线程");
			CmppSubmitMsgThread.running=true;
			new Thread(cmppSubmitMsgThread).start();
		}
